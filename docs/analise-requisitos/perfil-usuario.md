...
!!! info "Espaço Reservado para Desenvolvimento Individual (Gustavo Antonio)"
    Esta seção é de responsabilidade autoral exclusiva do discente **Gustavo Antonio Rodrigues e Silva**, sendo desenvolvida diretamente em sua respectiva branch temática (`feat/...`) para posterior submissão via Pull Request e revisão por Edvaldo Soares.
...
O Perfil 3 representa pessoas que acessam o Fórum Diolinux Plus para encontrar informações ou soluções para uma necessidade relacionada a Linux, software livre ou tecnologia. Antes de criar uma publicação, esse usuário pesquisa discussões existentes, examina os resultados e avalia se alguma resposta atende ao que precisa.

A busca autônoma é o foco deste perfil; ela não pressupõe que a pessoa conheça os filtros ou a sintaxe de pesquisa do Discourse. O nível de experiência com Linux, fóruns e ferramentas de busca pode variar e deve ser caracterizado com dados dos participantes. Barbosa e Silva (2010, p. 134–136; p. 174–176) recomendam caracterizar usuários, sua relação com tecnologia, conhecimento do domínio e tarefas, refinando os perfis iterativamente com dados coletados.

A Tabela 2 apresenta a caracterização preliminar. Informações ainda não confirmadas estão identificadas como hipóteses ou dados a levantar.

**Tabela 2** — Caracterização preliminar do Perfil 3 (Pesquisador / Buscador Autônomo)

| Dimensão de Análise | Atributo Investigado | Caracterização preliminar |
| :--- | :--- | :--- |
| **Dados Demográficos** | Faixa etária, ocupação e escolaridade | A levantar com participantes; não há dados disponíveis para definir faixas ou distribuição. |
...
| **Conhecimento do Domínio** | Conhecimento sobre o assunto pesquisado | Pode variar conforme a dúvida. A pessoa pode estar aprendendo ou buscando informação específica em área que já conhece. |
...
| **Atitudes com a Tecnologia** | Motivação de acesso | **Hipótese a validar:** resolver uma necessidade ou aprender sobre um assunto consultando informações existentes antes de perguntar à comunidade. |
...
| **Tarefas Primárias no Fórum** | Busca de informação | Formular consulta relacionada à necessidade e examinar os resultados. |
| | Avaliação dos resultados | Comparar títulos, categorias, datas e conteúdo das discussões para escolher quais parecem relevantes. |
...
| **Necessidades de Interação** | Compreensão dos resultados | Títulos informativos e contexto suficiente para decidir quais resultados abrir. |
| | Refinamento da consulta | Filtros e opções de ordenação compreensíveis, com indicação clara de quais estão ativos. |
| | Avaliação da informação | Informações que ajudem a avaliar relevância e atualidade, como data, categoria e eventual indicação de solução aceita, se disponível. |
...
| | Resultado irrelevante ou desatualizado | Pode gerar retrabalho, orientação inadequada ou desistência; frequência e gravidade devem ser investigadas. |
| **Frequência e Duração** | Frequência e duração das buscas | A levantar por entrevista ou observação; não devem ser presumidas. |
...
_Fonte: Elaborada por [Edvaldo Soares Brasileiro Filho](https://github.com/PajeMurici-dev), 2026. Caracterização preliminar, sujeita à validação com usuários._

Conforme sintetizado na Tabela 2, este perfil precisa localizar discussões pertinentes, compreender os resultados e avaliar se uma resposta atende à sua necessidade. As entrevistas e a observação da tarefa de busca devem confirmar ou revisar as hipóteses, especialmente quanto à experiência com Linux, aos hábitos de pesquisa, ao uso de filtros e aos critérios de confiança nas respostas.
...
| **Experiência em Linux** | Avançada (Kernel, terminal e scripts) | A levantar | Variável; a levantar |
| **Postura com a Tecnologia** | Tecnófilo | A levantar | Hipótese: busca autônoma; validar |
| **Motivação de Acesso** | Compartilhar e aprofundar conhecimento | A levantar | Hipótese: localizar informação antes de perguntar; validar |
| **Fluxo Principal no Fórum** | Criação de tópico com Markdown | A levantar | Busca, avaliação e refinamento de resultados |

_Fonte: Elaborada pelos autores, 2026._

A matriz será revisada após a consolidação do Perfil 2 e a validação dos dados empíricos dos três perfis.
...
Durante a preparação deste artefato, foi utilizado suporte de Inteligência Artificial Generativa para estruturar o rascunho preliminar do Perfil 3 e auxiliar na formatação Markdown. As hipóteses desse perfil ainda precisam ser revisadas pelo autor e validadas com usuários; não são apresentadas como resultados empíricos.
